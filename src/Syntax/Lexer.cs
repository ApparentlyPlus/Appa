...
{
    private int _pp;  // next character to read
    private int _ts;  // where the current token started
    ...
    private void ReadOne()
    {
        ...
        if (Cur == '@')
        {
            ...
            switch (nn)
            {
                ...
                case "shadows": Emit(TK.AtShadows, "@shadows"); return;
                case "builtin": Emit(TK.AtBuiltin, ReadParenArg("@builtin")); return;
                default:
                    Fail($"unknown annotation '@{nn}'; expected '@intrinsic', '@preamble', '@extern', " +
        ...
        switch (Cur)
        {
            ...
            case ':':
                ...
                if (Peek() == ':') { Advance(2); Emit(TK.ColonColon, "::"); return; }
                break;
        }
    ...
    private string ReadBalanced()
    {
        ...
        while (_pp < src.Length && depth > 0)
        {
            ...
            // A brace inside a comment or a literal must not move the depth.
            if (cur == '/' && peek == '/')
            {
                while (_pp < src.Length && Cur != '\n') Advance();
            }
        ...
        while (_pp < src.Length && Cur != '"' && Cur != '\n')
        {
            if (Cur == '{' && Peek() != '{')
            {
                ...
                while (_pp < src.Length && brdepth > 0)
                {
                    ...
                    ReadOne();  // ordinary tokens, recursively
                }
