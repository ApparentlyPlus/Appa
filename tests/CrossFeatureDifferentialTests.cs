...
///
/// Each fragment is a Gata expression transforming an int, paired with a C# function computing the
/// same thing. Random chains are composed into one program of many independent cases, run once
/// against the real standard library, and every printed answer is compared with the C# evaluation.
/// The point is the crossing - a feature works in isolation in its own test file, and this asks
/// whether it still works with four others stacked on top of it.
///
/// Everything is in one generated program on purpose: one build and one run covers hundreds of
/// cases, which is what makes a sweep this wide affordable in CI.
/// </summary>
public class CrossFeatureDifferentialTests
{
    private sealed record Frag(
        string Name,
        string Decls,                    // top-level declarations this fragment needs
        Func<string, string> Apply,      // Gata expression transforming the input expression
        Func<int, int> Eval);            // the same transformation in C#

    private static List<Frag> Frags() =>
    [
        new("add",       "", x => $"(({x}) + 7)",                        v => v + 7),
        new("mul",       "", x => $"(({x}) * 3)",                        v => v * 3),
        new("ternary",   "", x => $"((({x}) % 2) == 0 ? (({x}) / 2) : (({x}) * 2 + 1))",
                                                                         v => v % 2 == 0 ? v / 2 : v * 2 + 1),
        new("cast64",    "", x => $"((({x}) as int64) * (2 as int64)) as int",
                                                                         v => (int)((long)v * 2L)),
        new("castdouble","", x => $"(((({x}) as double) * 1.5) as int)",  v => (int)((double)v * 1.5)),
        new("charmath",  "", x => $"(((('a' as int) + ({x}) % 5) as char) as int)",
                                                                         v => 'a' + v % 5),

        new("method", "class Acc { public int n; func _init(int s) { self.n = s; } " +
                      "public int func Bump(int d) { self.n = self.n + d; return self.n; } }",
            x => $"NewAcc({x})",                                          v => v + 4),

        new("genclass", "class Hold[T] { public T v; func _init(T x) { self.v = x; } " +
        ...
        new("union", "union Sig { Off, On(int lvl) }\n" +
            ...
            x => $"Weigh(((({x}) % 3) == 0 ? Sig.Off() : Sig.On({x})))",
                                                                          v => v % 3 == 0 ? 0 : v + 1),

        new("enumsw", "enum Col { Red, Green, Blue }\n" +
                      "int func ColVal(Col c) { switch (c) { case Col.Red { return 1; } " +
                      "case Col.Green { return 2; } default { return 3; } } }",
            x => $"ColVal(((({x}) % 2) == 0 ? Col.Red : Col.Blue))",
                                                                          v => v % 2 == 0 ? 1 : 3),

        new("funcptr", "int func Tripler(int a) { return a * 3; }",
            x => $"CallFp(Tripler, {x})",                                 v => v * 3),

        // A handler is root-position only, so these go through helpers rather than nesting inline.
        new("throwsok", "throws int func MayFail(int a) { if (a < 0) { throw; } return a + 5; }\n" +
                        "int func CatchOk(int a) { let int r = MayFail(a) catch { assign -1; }; return r; }",
            x => $"CatchOk({x})",                                          v => v >= 0 ? v + 5 : -1),

        new("throwsfail", "throws int func AlwaysFail(int a) { if (a > -999999) { throw; } return a; }\n" +
                          "int func CatchFail(int a) { let int r = AlwaysFail(a) catch { assign a + 2; }; return r; }",
            x => $"CatchFail({x})",                                        v => v + 2),

        new("trycatch", "throws int func MayFail2(int a) { if (a < 0) { throw; } return a + 6; }",
            x => $"TryIt({x})",                                            v => v >= 0 ? v + 6 : -2),

        new("deferloop", "", x => $"DeferSum({x})",                        v => { int s = 0; for (int i = 0; i < 3; i++) s += v + i; return s; }),

        new("recurse", "", x => $"Fib(((({x}) % 10) + 10) % 10)",
            v => { int n = ((v % 10) + 10) % 10; int a = 0, b = 1; for (int i = 0; i < n; i++) (a, b) = (b, a + b); return a; }),

        new("refparam", "void func AddTo(ref int slot, int d) { slot = slot + d; }",
            x => $"ViaRef({x})",                                           v => v + 9),

        new("array", "", x => $"ArrSum({x})",                              v => v + (v + 1) + (v + 2) + (v + 3)),

        new("oper", "class Vec { public int n; func _init(int a) { self.n = a; } " +
    ...
    /// <summary>Helpers the fragments call, kept out of the fragment table so it stays readable.</summary>
    ...
    /// <summary>Builds N cases, each a chain of `depth` fragments applied to a seed.</summary>
    private static List<GenCase> Generate(int count, int depth, int seed)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            int val = start;
            // Bounded after every step. Signed overflow is UB in C, so an unbounded chain would
            // have UBSan reporting the harness rather than the compiler.
    ...
    public void CrossFeatureDifferential()
    {
        ...
        // All fragment declarations once, then one function per case: a case's chain can name any
        // helper, and duplicate declarations would collide.
